...
/**
 * This test checks that 'Frost Nova' is working correctly.
 */
public class FrostNovaTest {

	@Test
	public void testPlayFrostNova() {
		final Player player1 = new Player()
			.setHitPoints(30)
			.setAvailableMana(3)
			.addToHand(CardRepository.createByName("Frost Nova"));
		...
		Assert.assertEquals("player two, minions on board", 2, afterState.getPlayer(PlayerOrdinal.Two).getBattlefield().size());
		Assert.assertTrue("player two, minion one, is frozen", afterState.getPlayer(PlayerOrdinal.Two).getBattlefield().get(0).isFrozen());
		Assert.assertTrue("player two, minion two, is frozen", afterState.getPlayer(PlayerOrdinal.Two).getBattlefield().get(1).isFrozen());
		// end the turn.
		HearthstoneGameState nextState = new ActionFactory().createAllApplicableActions(afterState).get(0).apply(afterState);
		Assert.assertTrue("player two, minion one, is frozen", nextState.getPlayer(PlayerOrdinal.Two).getBattlefield().get(0).isFrozen());
		Assert.assertTrue("player two, minion two, is frozen", nextState.getPlayer(PlayerOrdinal.Two).getBattlefield().get(1).isFrozen());
		// end the turn.
		HearthstoneGameState finalState = new ActionFactory().createAllApplicableActions(nextState).get(0).apply(nextState);
		Assert.assertFalse("player two, minion one, is no longer frozen", finalState.getPlayer(PlayerOrdinal.Two).getBattlefield().get(0).isFrozen());
		Assert.assertFalse("player two, minion two, is no longer frozen", finalState.getPlayer(PlayerOrdinal.Two).getBattlefield().get(1).isFrozen());
	}
}
